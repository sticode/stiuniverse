...
const std::string GuiResourceFile::FILENAME = "gui.res";

GuiResourceFile::GuiResourceFile()
{
...
void GuiResourceFile::addResource(std::string r_name, std::string r_path)
{
	resourcesPath.insert(std::make_pair(r_name, r_path));
}


Surface* GuiResourceFile::getSurface(std::string r_name)
{
	resources[r_name];
}

void GuiResourceFile::save(std::string destination)
{
...
void GuiResourceFile::load(std::string source)
{
